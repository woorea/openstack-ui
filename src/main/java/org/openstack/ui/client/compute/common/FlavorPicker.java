...
public class FlavorPicker extends PopupPicker<String> {
	...
	public FlavorPicker() {
		...
		Portal.CLOUD.listFlavors(0, 10,  new AsyncCallback<FlavorList>() {
			...
			@Override
			public void onSuccess(final FlavorList result) {
				
				setOptions(result.getList());
	...
	public void setOptions(final List<Flavor> flavors) {
		...
		for (Flavor flavor : flavors) {
			...
			grid.getRowFormatter()
					.getElement(row)
					.setAttribute("style",
							"border-bottom: 1px solid #EAEAEA; cursor: pointer");
			row++;
		}
